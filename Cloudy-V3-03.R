...
## ARGUMENTS ###############
## 'drp' = numeric vector of all (endpoint) fluorescence measurments in a digital reaction
...
## 'neg.ref' = is a numerical of length 1. This provides a reference where the algorithm
...
## PLOT 3: when applicable, shows the effect of the cross-talk correction (2D droplet plots: before & after correction)

## Version history: this is stand alone version 3.06
...
## Main function ###############
#########################################################################################################
cloudy <- function(drp, method = c("simplex", "simplex2", "eva", "duplex")[1], neg.ref = 13500, threshold = NA,
...
if(is.matrix(drp$Ch1)){
  ...
  for(s in 1:ncol(drp$Ch1)){
    ...
    multiput[, s] <- cloudy(subdrp, method, dVol, sVol, plots, silent, vec = TRUE, threshold)
  }
  return(multiput)
}else{
##################################################################################
...
##################################################################################
checks <- c(length(method == 1), method %in% c("simplex", "simplex2", "eva", "duplex"), length(c(dVol, sVol) == 2), is.numeric(c(dVol, sVol)),
             is.logical(c(plots, silent, vec)), length(c(plots, silent, vec)) == 3, is.na(threshold) | is.numeric(threshold))
